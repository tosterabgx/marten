...
// Names are taken from https://github.com/moby/moby/blob/master/internal/namesgenerator/names-generator.go
...
func GetAvailableName(conn *net.Conn) (string, error) {
	...
	for range maxAttempts {
		...
		subdomainMu.Lock()
		_, ok := subdomainTunnels[subdomain]
		if !ok {
	...
	return "", fmt.Errorf("no available subdomain after %d attempts", maxAttempts)
}
